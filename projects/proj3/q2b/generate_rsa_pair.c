...
void read_file(char **key, FILE *fptr);
...
int main (int argc, char *argv[])
{
    ...
    char *public_key_sample = 0;
    char *public_key_generated = 0;
    char *private_key_generated = 0;
    FILE *fptr = fopen("server_pubkey.pub", "r");
    read_file(&public_key_sample, fptr);
    
    RSA *rsa_short;

    for (int i = 0; i < (UINT_MAX >> 7); i++) {

	// printf("%d\n\n", i);

	srand(i);
	rsa_short = RSA_generate_key(num_bits,exponent,NULL,NULL);
	...
	// Write out the public key
        ...
        if (!PEM_write_RSAPublicKey(fp, rsa_short))
        {
	...
	read_file(&public_key_generated, fp);
	// printf("%s\n\n", public_key_generated);

	if (strcmp(public_key_sample, public_key_generated) == 0) {
	    ...
	    read_file(&private_key_generated, fp);
	    printf("%s", private_key_generated);

	    free(private_key_generated);
	    break;
	}

	free(public_key_generated);

    }

    free(public_key_sample);
...
/*
    ...
    // BN_cmp().
    // https://www.openssl.org/docs/man1.0.2/crypto/BN_cmp.html
