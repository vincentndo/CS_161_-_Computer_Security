...
def manipulate(client_ip,client_port,server_ip,server_port,http_request):
  index_1 = http_request.find("Content-Length: ")
  start = index_1 + 16
  end = index_1 + 18
  old_length = int( http_request[start:end] )
  new_length = old_length - 1
  http_request = http_request.replace("Content-Length: " + str(old_length), "Content-Length: " + str(new_length))
